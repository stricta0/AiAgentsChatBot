...
public class UnknownResolutionPromptDefinition {
    ...
    private static final String DEFAULT_RESOURCE_PATH = "router/unknown_resolution_prompt.json";
    ...
    public static UnknownResolutionPromptDefinition load() {
        ObjectMapper objectMapper = new ObjectMapper();

        try (InputStream inputStream = getResourceAsStream(DEFAULT_RESOURCE_PATH)) {
            return objectMapper.readValue(inputStream, UnknownResolutionPromptDefinition.class);
        } catch (IOException e) {
            throw new IllegalStateException(
                    "Failed to load unknown resolution prompt definition from " + DEFAULT_RESOURCE_PATH, e
            );
        }
    }
